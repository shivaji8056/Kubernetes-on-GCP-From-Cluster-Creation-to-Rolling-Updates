...
*   The Google Cloud SDK (`gcloud`) installed and configured on your local machine.  Follow the official [GCP documentation](https://cloud.google.com/sdk/docs/install) for installation instructions.
*   The `kubectl` command-line tool installed on your local machine.  Install it using `gcloud components install kubectl`.
...
*   The cluster creation process will take several minutes.  Grab a coffee! ☕
...
Once the cluster is created, click the "CONNECT" button in the GCP Console.
Choose "Run in Cloud Shell" or copy the command to run locally:
...
This command should display the nodes in your cluster. If you see them, you're connected! 🎉

3. Deploying a "Hello World" Application
Create a Deployment File (hello-app.yaml): Create a file named hello-app.yaml with the following content:
apiVersion: apps/v1
kind: Deployment
metadata:
  name: hello-app
...
Wait for the EXTERNAL-IP to be assigned to the hello-app-service. This may take a few minutes.

Access the Application: Open the EXTERNAL-IP in your web browser. You should see the "Hello World" application. Hello, World! 🌎
4. Performing Rolling Updates

Rolling updates allow you to update your deployments without downtime.

Update the Deployment Configuration: Edit the hello-app.yaml file and change the image version to gcr.io/google-samples/hello-app:2.0:
spec:
  containers:
  - name: hello-app
...
This command will show you the progress of the rolling update. Watch as Kubernetes gracefully updates your application!

Verify the Update:
kubectl get pods -l app=hello-app


You should see new pods with the updated image version. Congratulations, you've performed a rolling update! 🎉
...
kubectl exec -it <pod-name> -- /bin/bash: Get a shell inside a pod. Useful for debugging!
...
Recreate: Terminates all existing pods before creating new ones. This strategy causes downtime. Use with caution!
...
This document has provided a basic introduction to creating a Kubernetes cluster on GCP, deploying a simple application, and performing rolling updates. By following these steps, you can start building and deploying your own applications on Kubernetes. Happy Kubernetes-ing! 🚀
